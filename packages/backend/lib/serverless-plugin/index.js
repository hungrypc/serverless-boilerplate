...
function applyWebpackOptions(custom, config) {
  ...
  custom.webpack = {
    ...
    includeModules: {
      ...
      packagePath: path.relative(config.servicePath, pkgUp.sync({ cwd: config.servicePath })),
    },
...
function applyUserConfig(config, userConfig, servicePath, runtime) {
  ...
  if (userConfig.forceExclude) {
    userConfig.forceExclude = config.options.forceExclude.concat(userConfig.forceExclude)
  }

  // Concat externals if a list of packages are provided
  if (userConfig.externals && Array.isArray(userConfig.externals)) {
    userConfig.externals = config.options.externals.concat(userConfig.externals)
  }

  // Concat externals if provided
  if (userConfig.rawFileExtensions) {
    userConfig.rawFileExtensions = config.options.rawFileExtensions.concat(userConfig.rawFileExtensions)
  }

  Object.assign(config.options, userConfig)

  // Default to Node 10 if no runtime found
  config.nodeVersion = Number.parseInt((runtime || '').replace('nodejs', ''), 10) || 10
}
...
class ServerlessPlugin extends ServerlessWebpack {
  constructor(serverless, options) {
    ...
    this.hooks['before:webpack:validate:validate'] = function () {
      ...
      applyUserConfig(config, service.custom.bundle || {}, servicePath, service.provider.runtime)
